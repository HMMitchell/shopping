<template>
    <div class="center">
        <div class="el-row" v-if="orderinfo">
                                ...
                                <div class="bg-wrap" style="min-height: 765px;">
                                    ...
                                    <div class="order-progress">
                                        <ul>
                                            ...
                                            <li :class='{active:orderinfo.status!=1&&orderinfo.status!=2}'>
                                                <div class="progress">已经发货</div>
                                                <div class="info">{{orderinfo.express_time | cutTime('YYYY-MM-DD hh:mm:ss')}}</div>
                                            </li>
                                            <li class="last" :class='{active:orderinfo.status!=1&&orderinfo.status!=2}'>
                                    ...
                                    <div class="form-box accept-box form-box1">
                                        <dl class="head form-group">
                                            <dd>
                                                ...
                                                <a v-on:click="sign(orderid)" v-if="orderinfo.status==2" class="btn-pay">去签收</a>
                                                <a v-if="orderinfo.status==4" class="btn-pay">已签收</a>
                                                <!---->
                                            </dd>
                                        </dl>
                                        <dl class="form-group">
...
export default {
  ...
  computed: {
    totalPrice() {
      ...
      this.goodslist.forEach(v => {
        totalPrice += v.goods_price * v.quantity;
      });
      return totalPrice;
    }
  },
  methods: {
    ...
    sign(val) {
      //   console.log(val);
      this.axios
        .get(`site/validate/order/complate/${val}`)
        .then(response => {
          //  console.log(response)
